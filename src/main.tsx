...
function Library({ dashboard, plugins, run }: { dashboard: Dashboard; plugins: Plugin[]; run: (op: () => Promise<unknown>, msg: string) => Promise<void> }) {
  ...
  // Auto-record only applies to performers connected to a live-cam provider.
  const liveCamProviders = new Set(plugins.filter((plugin) => plugin.installed && plugin.enabled && plugin.manifest.capabilities.includes("live-cam")).map((plugin) => plugin.manifest.id));
...
function PerformerManager({ performer, sources, items, plugins, close, run }: { performer: Performer; sources: Source[]; items: Item[]; plugins: Plugin[]; close: () => void; run: (op: () => Promise<unknown>, msg: string) => Promise<void> }) {
  ...
  // Auto-record is a performer-level flag backed by live-cam favorites; show its toggle
  // only on URL rows whose plugin can actually host live cams.
  const liveCamProviders = new Set(plugins.filter((plugin) => plugin.installed && plugin.enabled && plugin.manifest.capabilities.includes("live-cam")).map((plugin) => plugin.manifest.id));
...
function SettingsPage({ settings, run, setNotice, onSettingsChange }: { settings: Record<string, any>; run: (op: () => Promise<unknown>, msg: string) => Promise<void>; setNotice: (text: string) => void; onSettingsChange: (settings: Record<string, unknown>) => void }) {
  ...
  const saveAutomation = () => void run(() => api("/api/settings", { method: "PUT", body: JSON.stringify({ defaultScrapeIntervalMinutes: values.defaultScrapeIntervalMinutes, defaultLiveIntervalSeconds: values.defaultLiveIntervalSeconds, autoRecordCheckSeconds: values.autoRecordCheckSeconds, maxConcurrentDownloads: values.maxConcurrentDownloads, autoQueueDiscovered: values.autoQueueDiscovered, retentionDays: values.retentionDays }) }), "Settings saved");
