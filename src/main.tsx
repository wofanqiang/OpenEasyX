...
function SettingsPage({ settings, run, setNotice, onSettingsChange }: { settings: Record<string, any>; run: (op: () => Promise<unknown>, msg: string) => Promise<void>; setNotice: (text: string) => void; onSettingsChange: (settings: Record<string, unknown>) => void }) {
  ...
  const saveAutomation = () => void run(() => api("/api/settings", { method: "PUT", body: JSON.stringify({ defaultScrapeIntervalMinutes: values.defaultScrapeIntervalMinutes, defaultLiveIntervalSeconds: values.defaultLiveIntervalSeconds, autoRecordCheckSeconds: values.autoRecordCheckSeconds, maxConcurrentDownloads: values.maxConcurrentDownloads, maxConcurrentRecordings: values.maxConcurrentRecordings, minFreeDiskGb: values.minFreeDiskGb, autoQueueDiscovered: values.autoQueueDiscovered, retentionDays: values.retentionDays }) }), "Settings saved");
